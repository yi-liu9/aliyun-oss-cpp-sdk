        ...
        if (root && !std::strncmp("ListInventoryConfigurationsResult", root->Name(), 33)) {
            ...
            for (; node; node = node->NextSiblingElement()) {
                ...
                if (conf_node) {
                    ...
                    if (prefix_node && prefix_node->GetText()) inventoryConfiguration_.setPrefix(prefix_node->GetText());
                }
                ...
                if (conf_node) {
                    ...
                    if (freq_node && freq_node->GetText()) inventoryConfiguration_.setSchedule(freq_node->GetText());
                }

                conf_node = node->FirstChildElement("IncludedObjectVersions");
                if (conf_node && conf_node->GetText()) inventoryConfiguration_.setIncludedObjectVersions(conf_node->GetText());
                ...
                if (conf_node) {
                    ...
                    for (; field_node; field_node = field_node->NextSiblingElement()) {
                        if (field_node->GetText())
                            field.push_back(field_node->GetText());
                    }
