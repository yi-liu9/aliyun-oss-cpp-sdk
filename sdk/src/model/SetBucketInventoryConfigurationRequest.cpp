...
ParameterCollection SetBucketInventoryConfigurationRequest::specialParameters() const
{
    ...
    parameters["id"] = inventoryConfiguration_.Id();
    return parameters;
}
...
std::string SetBucketInventoryConfigurationRequest::payload() const
{
    ...
    ss << "<Filter>" << std::endl;
    ss << "<Prefix>" << inventoryConfiguration_.Prefix() << "</Prefix>" << std::endl;
    ss << "</Filter>" << std::endl;
    ...
    ss << "<Frequency>" << inventoryConfiguration_.Schedule() << "</Frequency>" << std::endl;
    ss << "</Schedule>" << std::endl;
    ss << "<IncludedObjectVersions>" << inventoryConfiguration_.IncludedObjectVersions() << "</IncludedObjectVersions>" << std::endl;

    ss << "  <OptionalFields>" << std::endl;
    for (const auto& field : inventoryConfiguration_.OptionalFields()) {
        ss << "    <Field>" << field << "</Field>" << std::endl;
    }
    ss << "  </OptionalFields>" << std::endl;
